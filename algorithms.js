...
export const compareAlgorithms = (processes, quantum) => {
    ...
    Object.keys(algorithms).forEach(name => {
        // Use a deep copy for each algorithm run to avoid one algorithm
        // mutating the shared `processes` array and affecting others.
        const localProcesses = JSON.parse(JSON.stringify(processes));
        const { solvedProcesses } = algorithms[name](localProcesses, quantum);
